...
def generate_activities() -> None:
    """
     Generate activities where John Due is activity holder
    """
    ...
    for _ in range(10):
        call_entity = CallActivity(company_id=john_due.company_id,
                                   ...
                                   finish_until=combine_date_time(fake.future_date(),
...
def generate_activities_for_current_user() -> None:
    """
     Generate activities where John Due executor
    """
    john_due = Employee.query.filter(Employee.email == "john@example.com").first()
    all_employee_id = [
        employee.id for employee in Employee.query.filter(Employee.company_id == john_due.company_id).all()]
    ...
    for _ in range(10):
        call_entity = CallActivity(company_id=john_due.company_id,
                                   ...
                                   activity_holder=john_due.id,
                                   activity_holder_name=f"{john_due.first_name} {john_due.last_name}",
                                   executor=john_due.id,
                                   executor_name=f"{john_due.first_name} {john_due.last_name}",
                                   finish_until=combine_date_time(fake.future_date(),
                                                                  datetime.time(random.randint(0, 23),
                                                                                random.randint(0, 59),
                                                                                random.randint(0, 59))))
...
def generate_overdue_activities() -> None:
    """
     Generate overdue activities for John Due
    """
    john_due = Employee.query.filter(Employee.email == "john@example.com").first()
    result = []
    for _ in range(5):
        call_entity = CallActivity(company_id=john_due.company_id,
                                   ...
                                   executor=john_due.id,
                                   executor_name=f"{john_due.first_name} {john_due.last_name}",
                                   finish_until=combine_date_time(fake.past_date(),
                                                                  datetime.time(random.randint(0, 23),
                                                                                random.randint(0, 59),
                                                                                random.randint(0, 59))))
        meeting_entity = MeetingActivity(company_id=john_due.company_id,
